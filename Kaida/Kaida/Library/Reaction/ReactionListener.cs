...
namespace Kaida.Library.Reaction
{
    public class ReactionListener : IReactionListener
    {
        ...
        public bool IsListener(ulong id, DiscordEmoji emoji, BaseDiscordClient client)
        {
            ...
            return _listener.ContainsKey(id.ToString()) && _listener[id.ToString()]
               .ContainsKey(emoji);
        }

        public Task AddRoleToListener(ulong messageId, DiscordEmoji emoji, DiscordRole role, BaseDiscordClient client)
        {
            if (redis.SetMembers("MessageIds")
                     .All(e => e.ToString() != messageId.ToString()))
            {
                redis.SetAdd("MessageIds", messageId);
            }

            redis.HashSet($"Messages:{messageId}", new[] {new HashEntry(emoji.ToString(), role.Id.ToString())});

            if (_listener.ContainsKey(messageId.ToString()))
            {
                _listener[messageId.ToString()]
                   .Add(emoji, role.Id);

                return Task.CompletedTask;
            }

            var dictionary = new Dictionary<DiscordEmoji, ulong> {{emoji, role.Id}};
        ...
        {
            if (_listener.ContainsKey(messageId.ToString()) && _listener[messageId.ToString()]
               .ContainsKey(emoji))
            {
                redis.HashDelete($"Messages:{messageId}", emoji.ToString());
                _listener[messageId.ToString()]
                   .Remove(emoji);
        ...
        public async void ManageRole(DiscordChannel channel, DiscordMessage message, DiscordUser user, DiscordEmoji emoji, BaseDiscordClient client)
        {
            ...
            {
                logger.Information($"The role '{role.Name}' was revoked from '{user.Username}#{user.Discriminator}' on the guild '{channel.Guild.Name}' ({channel.Guild.Id}).");
                await ((DiscordMember) user).RevokeRoleAsync(role);
            }
            else
            {
                logger.Information($"The role '{role.Name}' was granted to '{user.Username}#{user.Discriminator}' on the guild '{channel.Guild.Name}' ({channel.Guild.Id}).");
        ...
        {
            var ids = redis.SetMembers("MessageIds");
            _listener = new Dictionary<string, Dictionary<DiscordEmoji, ulong>>();

            foreach (var id in ids)
            {
                var reactions = redis.HashGetAll($"Messages:{id}");
